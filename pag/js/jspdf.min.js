...
function printRow(row, drawRowHooks, drawCellHooks) {
    ...
    for (var i = 0; i < table.columns.length; i++) {
        ...
        var data = config_1.Config.hooksData({ column: column, row: row, addPage: common_1.addPage, pageSize: config_1.Config.pageSize() });
...
function DownloadSchedulePDF() {
  ...
  var res = GetTableJSON(document.getElementById("schedule"), false);
  console.log("1");
  console.log(res);
  ...
  doc.autoTable(res.columns, res.data, {
    ...
    drawCell: function(cell, data) {
      ...
      if (cell.raw.indexOf("*") > -1) {
        ...
        while((cell.height * times + cell.y) > data.pageSize.height) {
        	times--;
        }
...
function ApplyCellStyle(cell, x, y) {
  ...
  if (styles === undefined)
    return;
  cell.styles.cellPadding = styles.cellPadding || 0;
  if (pdfInColor) {
    cell.styles.fillColor = styles.fillColor;
    cell.styles.textColor = styles.textColor;
  } else {
    cell.styles.fillColor = cell.raw !== undefined && cell.raw !== '' ? [210, 210, 210] : [255, 255, 255];
    cell.styles.textColor = [0, 0, 0];
  }
  cell.styles.font = styles.font;
  cell.styles.fontStyle = styles.fontStyle;
  cell.styles.columnWidth = styles.columnWidth;
  cell.styles.rowHeight = styles.rowHeight;
  cell.styles.overflow = styles.overflow;
}

Object.vals = function(o) {
  return Object.values ? Object.values(o) : Object.keys(o).map(function(k) {
    return o[k];
  });
}
...
function GetTableJSON(tableElem, includeHiddenElements) {
  ...
  for (var k = 0; k < header.cells.length; k++) {
    ...
    cellsStyle["th-" + k].overflow = 'linebreak';
    if (includeHiddenElements || style.display !== 'none') {
    ...
    if (includeHiddenElements || style.display !== 'none') {
      ...
      for (var j in Object.keys(columns)) {
        ...
        if (style.display !== 'none') {
          ...
          cellsStyle[(i - 1) + "-" + j].overflow = 'visible';
        } else {
        	val = 'hidden'; 
        }
      	rowData.push(val);
  ...
  return {
    columns: Object.vals(columns),
    rows: rows,
    data: rows,
    styles: cellsStyle
  }; // data prop deprecated
};

function AdjustStyleProperties(style) {
  return {
    cellPadding: parseInt(style.padding),
    fontSize: parseInt(style.fontSize)-2,
...
function ConvertToRGB(value) {
  if (value === undefined || value === '' || value === "transparent")
    ...
    value = value.replace(/[^\d,]/g, '').split(',').map(function(x) {
      return parseInt(x)
    });
  else if (value.indexOf("#") > -1)
    value = value.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, function(m, r, g, b) {
      return '#' + r + r + g + g + b + b
    })
    ...
    value = imgData.data.slice ? imgData.data.slice(0, 3) : [imgData.data[0], imgData.data[1], imgData.data[2]];
  }
  return value;
}
